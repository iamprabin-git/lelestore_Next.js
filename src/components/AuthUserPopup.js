...
function AuthUserPopup({user}) {
  ...
  return (
      <div className="absolute top-10 right-0 bg-white shadow-lg p-4">
        ...
        <Link href="/profile" className="flex items-center gap-2" onClick={() => setShowpopup(false)}>Profile</Link>
        
      <button
        onClick={() => dispatch(logoutUser())}
        className="bg-slate-700 py-1 px-4 text-white rounded flex items-center gap-2"
      >
